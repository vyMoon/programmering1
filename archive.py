...
def printOptions():
  ...
  for option, number in convertationOptions.items():
    print(f'{number} = {option}')

def choseConvertation():
  printOptions()
  
  while True:
    inp = requestNumber('Choose a number ')

    if not inp in convertationOptions.values():
      print('not existent option. Try one more time')
      continue
...
def convertTemperature():
  convertation = choseConvertation()
  temperature = requestNumber('Provide temperature for convertation')

  fn = ( celsiusToFahrenheit 
    if convertation == convertationOptions[cToF] 
    else fahrenheitToCelsius
  )
...
def countPayout(years, interest, capital):
  return capital * (1 + interest / 100) * years


def countMaturityPayout():
  years = requestNumber('How many years')
  interest = requestNumber('What is the interest')
  deposit = requestNumber('What is deposit')

  return countPayout(years, interest, deposit)

# print(countMaturityPayout())

addKEy ='Lägga till en låt'
delKey = 'Ta bort en låt'
lstKey = 'Lista låtarna'
stopLey = 'Avsluta programmet'
...
def printMeny(meny):
  print('Meny:')
  for description, number in meny.items():
    print(f'{number} - {description}')

def choseAction(meny):
  while True:
    inp = requestNumber('Chose a number')

    if not inp in meny.values():
      print(f'Det finns ingen option {inp}. försök igen')
      continue
...
def addSong(list):
  list.append(
    input('vilket låt vill du lägga till ')
  )

def deleteSong(list):
  song = input("Vilket låt vill du ta bort? ")
  if song in list:
    list.remove(song)
  else:
    print(f'Det finns inget låt med namnet {song}')
...
def musicManager():
  ...
  printMeny(meny)
  while True:
    action = choseAction(meny)

    if action == meny[stopLey]:
      break
    if action == meny[addKEy]:
      addSong(songs)
    if action == meny[delKey]:
      deleteSong(songs)
    if action == meny[lstKey]:
      listSongs(songs)
...
def printM(options):
  print('Meny')
  for description, option in options.items():
    print(f'{option} - {description}')

def chooseOption(meny):
  while True:
    inp = requestNumber('chose an option. Use numbers')

    if inp in meny.values():
      return inp
    print(f'there is no option {inp}. try one more time')

selMessage = 'sell'
buyMessage = 'buy'

def convert(rate, operation):
  eur = requestNumber(f'How much euros you would like to sell', True)
  sek = eur * rate
  print(f'You need {sek} sek')

def sell(rate):
  eur = requestNumber('How much euros you would like to sell', True)
  sek = eur * rate


def buy(rate):
  eur = requestNumber('HOw much euros you would like to buy', True)
  sek = eur * rate
  print(f'You need {sek} sek')
...
def conversation():
  rate = 11
  printM(options)
  option = chooseOption(meny)
  print(option)

  if option == options[stopKey]:
    return
  if option == options[sellKey]:
    sell(rate)
  if option == options[buyKey]:
    buy(rate)
